<?php
    ...
    if(isset($_GET['delete'])) {
        ...
        $sql = "DELETE FROM client WHERE NUMCLI='".$_GET["delete"]."'";
        $sql2 = "DELETE FROM contrat WHERE NUMCLI='".$_GET["delete"]."'";
            ...
            <table class="tableau">
                <thead>
                    <tr>
                        ...
                        <th>Adresse postale du client</th>
                ...
                <tbody>
                    <?php
                        ...
                        while($row = mysqli_fetch_assoc($query)) {
                            echo '<tr><td>' .$row["NUMCLI"]. '</td>';
                            echo '<td>' .$row["CODECOM"]. '</td>';
                            echo '<td>' .$row["RAISONSOCIALECLI"]. '</td>';
                            echo '<td>' .$row["TELCLI"]. '</td>';
                            echo '<td>' .$row["ADRESSEPOSTALECLI"]. '</td>';
                            echo '<td>' .$row["EMAILCLI"]. '</td>';
                            echo '<td><a href="update_client.php?update='.$row["NUMCLI"].'" class="btn"><img src="image/bouton-modifier.png"></a></td>';
                            echo '<td><a href="?delete='.$row["NUMCLI"].'" class="btn"><img src="image/supprimer.png"></a></td>';
                            '</tr>';
                        }
                    ?>
                </tbody>
