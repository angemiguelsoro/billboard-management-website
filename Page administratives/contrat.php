<?php
    ...
    if(isset($_GET['delete'])) {
        ...
        $sql = "DELETE FROM contrat WHERE NUMCONT='".$_GET["delete"]."'";
        $sql1 = "DELETE FROM appartenir WHERE NUMCONT='".$_GET["delete"]."'";
            ...
            <table class="tableau">
                <thead>
                    <tr>
                        ...
                        <th>Date signature de contat</th>
                        <th>Modifier</th>
                        <th>Supprimer</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                        ...
                        $query = mysqli_query($conn,"select *from contrat");

                        while($row = mysqli_fetch_array($query,MYSQLI_ASSOC)) {
                            echo '<tr><td>' .$row["NUMCONT"]. '</td>';
                            echo '<td>' .$row["NUMFACT"]. '</td>';
                            echo '<td>' .$row["NUMCLI"]. '</td>';
                            echo '<td>' .$row["DATESIGNATURECONT"]. '</td>';
                            echo '<td><a href="update_contrat.php?update='.$row["NUMCONT"].'" class="btn"><img src="image/bouton-modifier.png"></a></td>';
                            echo '<td><a href="?delete='.$row["NUMCONT"].'" class="btn"><img src="image/supprimer.png"></a></td>';
                            '</tr>';
                        }
                    ?>
                </tbody>
